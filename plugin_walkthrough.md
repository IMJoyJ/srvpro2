...
SRVPro2 提供了一个 **插件系统**，允许开发者在 `plugins/` 目录下放置编译好的 `.js` 文件来扩展功能，而不需要修改核心代码。
...
插件加载器位于 `src/plugin-loader.ts`，其工作流程为：

1. **扫描目录**：递归扫描 `plugins/` 目录下的所有 `.js` 和 `.cjs` 文件
...
- 文件必须以 `.js` 或 `.cjs` 结尾（TypeScript 需先编译）
- 文件放置在 `plugins/` 目录下，支持任意深度的子目录
...
在 `plugins/` 目录下创建一个 TypeScript 文件（或新建一个独立项目）：

```typescript
// plugins/my-plugin/src/index.ts
import { DefinePlugin, BasePlugin } from 'srvpro2/dist/src/utility/base-plugin';
...
将 TypeScript 编译为 JavaScript，确保输出到 `plugins/` 目录下：

```bash
# 如果是独立项目
tsc --outDir ../../plugins/my-plugin/dist

# 或者直接手动将编译产物放入 plugins/ 目录
```
...
```typescript
import { RoomManager } from 'srvpro2/dist/src/room';
...
```typescript
// 在你的插件文件顶部
declare module 'srvpro2/dist/src/room' {
  interface Room {
...
```typescript
declare module 'srvpro2/dist/src/client' {
  interface Client {
...
由于插件是外部加载的，你无法直接修改 `config.ts` 中的 `defaultConfig`。建议在插件中使用环境变量或自带配置文件的方式：
...
你也可以在 `config.yaml` 中添加自定义字段（它们不会影响核心功能）。
...
```typescript
import { I18nService } from 'srvpro2/dist/src/client';
...
- 实体文件必须以 **`.entity.js`** 结尾
- 文件放置在 `plugins/` 下任意位置
- 加载器（`plugin-typeorm-entity-loader.ts`）在 TypeORM DataSource 初始化时扫描这些文件
- 实体类必须使用 TypeORM 的 `@Entity()` 装饰器

### 示例

```typescript
// plugins/my-plugin/src/my-record.entity.ts
import { Entity, PrimaryGeneratedColumn, Column, CreateDateColumn } from 'typeorm';
import { BaseTimeEntity } from 'srvpro2/dist/src/utility/base-time.entity';
...
编译后产出 `my-record.entity.js`，放在 `plugins/` 目录下即可被自动加载。
...
```typescript
// plugins/enhanced-welcome/src/index.ts
import { DefinePlugin, BasePlugin } from 'srvpro2/dist/src/utility/base-plugin';
import { OnRoomJoin } from 'srvpro2/dist/src/room';
import { ChatColor } from 'ygopro-msg-encode';
import type { Context } from 'srvpro2/dist/src/app';
...
```typescript
// plugins/chat-reply/src/index.ts
import { DefinePlugin, BasePlugin } from 'srvpro2/dist/src/utility/base-plugin';
...
```typescript
// plugins/match-logger/src/match-log.entity.ts
import { Entity, PrimaryGeneratedColumn, Column, CreateDateColumn } from 'typeorm';
...
```typescript
// plugins/match-logger/src/index.ts
import { DefinePlugin, BasePlugin } from 'srvpro2/dist/src/utility/base-plugin';
import { OnRoomWin } from 'srvpro2/dist/src/room';
...
> 注意：`match-log.entity.ts` 编译后的 `match-log.entity.js` 会被自动扫描并注册到 TypeORM。
